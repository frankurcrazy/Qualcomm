...
function ParseArguments()
{
   ...
   var Txt = "";
   var Help =
      "Syntax: CScript HSDPACQI.js <ISF File Name> [Excel File Name]\n"
    + "Eg:     CScript HSDPACQI.js \"HSDPA CQI.isf\" \"HSDPA CQI.xls\"\n";
...
// Perform analysis (obtain cqi data from ISF file)
function Analyze()
{
   // Assume failure
   var RC  = false;
   ...
   // Configure the client for log 0x420B
   IConfig.AddLog( 0x420B );
   IConfig.CommitConfig();

   // Populate the client with all instances of log 0x420B
   ...
   for (var ItemIndex = 0; ItemIndex < ItemCount; ItemIndex++)
   {
      ...
      // Index to "Uncoded Quantized CQI" field
      var FieldIndex = 2;

      for (var s = 0; s < SamplesCount; s++, FieldIndex += 13)
      {
         var CQI = Fields.GetFieldValue( FieldIndex );
         if (CQI <= MAX_CQI)
         {
            gCQIArray[CQI] += 1;
            gSamples++;
...
function PopulateExcelSpreadsheet( Sheet )
{
   ...
   Sheet.Cells( 2, 1 ).Value = "CQI Index";
   Sheet.Cells( 2, 2 ).Value = "CQI Total";
   Sheet.Cells( 2, 3 ).Value = "CQI %";
   Sheet.Range( "A2:C2" ).Font.Bold = true;
   Sheet.Range( "A2:C2" ).ColumnWidth = 16;

   var CQI;
   var Percentage;
   for (CQI = 0; CQI <= MAX_CQI; CQI++)
   {
      Sheet.Cells( CQI + 3, 1 ).Value = CQI;
      Sheet.Cells( CQI + 3, 2 ).Value = gCQIArray[CQI];

      Percentage = gCQIArray[CQI] / gSamples * 100.0;
      Sheet.Cells( CQI + 3, 3 ).Value = Percentage;
   }

   var Range = Sheet.Range( "C2:C33" );
   if (Range == null)
   {
      return RC;
   }
   ...
   Chart.ChartTitle.Characters.Text = "HSDPA CQI";
   Chart.Axes( 1, 1 ).AxisTitle.Characters.Text = "CQI Index";
