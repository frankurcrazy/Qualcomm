...
# i.e.  Perl WCDMASoftHandovers.pl <ISF File Name> [Excel File Name]
...
sub ParseArguments
{
   ...
   my $Txt = "";
   my $Help =
      "Syntax: Perl WCDMASoftHandovers.pl <ISF File Name> [Excel File Name]\n"
    . "Eg:     Perl WCDMASoftHandovers.pl \"WCDMA Handover.isf\" \"WCDMA Handover.xls\"\n";
...
sub Analyze
{
   ...
   # Initialize array
   for (my $Cells = 1; $Cells <= MAX_CELLS; $Cells++)
   {
   ...
   for (my $ItemIndex = 0; $ItemIndex < $ItemCount; $ItemIndex++)
   {
      ...
      my $ActiveSets = $Fields->GetFieldValue( 0 );
      ...
      for (my $s = 0; $s < $ActiveSets; $s++, $FieldIndex += 8)
      {
         my $TPC = $Fields->GetFieldValue( $FieldIndex );
         if (!defined $TPCs{$TPC})
         {
            $UniqueTPCs++;
            $TPCs{$TPC} = true;
...
sub PopulateExcelSpreadsheet
{
   ...
   $Chart->Axes( 1, 1 )->AxisTitle->Characters->{Text} = "Percentage (%)";
