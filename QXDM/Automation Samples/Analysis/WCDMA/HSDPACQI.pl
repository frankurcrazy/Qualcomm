...
# i.e.  Perl HSDPACQI.pl <ISF File Name> [Excel File Name]
...
sub ParseArguments
{
   ...
   my $Txt = "";
   my $Help =
      "Syntax: Perl HSDPACQI.pl <ISF File Name> [Excel File Name]\n"
    . "Eg:     Perl HSDPACQI.pl \"HSDPA CQI.isf\" \"HSDPA CQI.xls\"\n";
...
sub Analyze
{
   ...
   for (my $ItemIndex = 0; $ItemIndex < $ItemCount; $ItemIndex++)
   {
      ...
      for (my $s = 0; $s < $SamplesCount; $s++, $FieldIndex += 13)
      {
         my $CQI = $Fields->GetFieldValue( $FieldIndex );
         if ($CQI <= MAX_CQI)
         {
            $gCQIArray[$CQI] += 1;
            $gSamples++;
