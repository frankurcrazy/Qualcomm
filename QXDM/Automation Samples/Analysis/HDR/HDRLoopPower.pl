...
# i.e.  Perl HDRLoopPower.pl <ISF File Name> [Excel File Name]
...
sub ParseArguments
{
   ...
   my $Txt = "";
   my $Help =
      "Syntax: Perl HDRLoopPower.pl <ISF File Name> [Excel File Name]\n"
    . "Eg:     Perl HDRLoopPower.pl \"HDR Power.isf\" \"HDR Power.xls\"\n";
...
sub Analyze
{
   ...
   for (my $ItemIndex = 0; $ItemIndex < $ItemCount; $ItemIndex++)
   {
      ...
      for (my $Frame = 1; $Frame <= 2; $Frame++, $FieldIndex += 9)
      {
         ...
         my $OpenLoopPwr = $Fields->GetFieldValue( $FieldIndex + 1 );
         my $ClosedLoopPwr = $Fields->GetFieldValue( $FieldIndex + 2 );

         # Convert to dBm and store
         $OpenLoopPwr = $OpenLoopPwr / 256.0;
         $OpenLoopPwr = sprintf( "%.6f", $OpenLoopPwr );
         $OpenLoopPwr = Round( $OpenLoopPwr );
         ...
         $ClosedLoopPwr = sprintf( "%.6f", $ClosedLoopPwr );
         $ClosedLoopPwr = Round( $ClosedLoopPwr );
...
sub PopulateExcelSpreadsheet
{
   ...
   $Chart->ChartTitle->Characters->{Text} = "HDR Loop Power";
