...
# i.e.  Perl HDRLockedFingers.pl <ISF File Name> [Excel File Name]
...
sub ParseArguments
{
   ...
   my $Txt = "";
   my $Help =
      "Syntax: Perl HDRLockedFingers.pl <ISF File Name> [Excel File Name]\n"
    . "Eg:     Perl HDRLockedFingers.pl \"HDR Finger.isf\" \"HDR Finger.xls\"\n";
...
sub Analyze
{
   ...
   for (my $ItemIndex = 0; $ItemIndex < $ItemCount; $ItemIndex++)
   {
      ...
      # Number of fingers
      my $FingCount = $Fields->GetFieldValue( 4 );
      if ($FingCount > MAX_FINGERS)
      {
         $FingCount = MAX_FINGERS;
      }

      for (my $Finger = 0; $Finger < $FingCount; $Finger++, $FieldIndex += 11)
      {
         my $PN = $Fields->GetFieldValue( $FieldIndex );
         if ($PN == 0xFFFF)
         {
            next;
         }

         my $Locked = $Fields->GetFieldValue( $FieldIndex + 3 );
         if ($Locked != 1)
         {
            next;
         }
         ...
         my $RSSI = $Fields->GetFieldValue( $FieldIndex + 2 );
         if ($RSSI == 0)
         {
            next;
         }
         ...
         $RSSI = 10.0 * (log( $RSSI / 512.0 ) / log(10.0));
...
sub PopulateExcelSpreadsheet
{
   ...
   my $Range = $Sheet->Range( "C2:C15, E2:E15, G2:G15, I2:I15" );
   if (!$Range)
   {
      return $RC;
   }
